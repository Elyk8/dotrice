...
myStartupHook = do
  ...
  spawn
    ("sleep 1 && trayer --edge top --align right --widthtype request --padding 5 --SetDockType true --SetPartialStrut true --expand true --transparent true --alpha 0 --tint 0x282c34 --monitor 1 --height 30 --iconspacing 4"
    )
...
myUpdatePointer refPos ratio = whenX isActive $ do
  ...
  where isActive = (\(MyUpdatePointerActive b) -> b) <$> XS.get
...
myXmobarPP s = filterOutWsPP [scratchpadWorkspaceTag] . marshallPP s $ def
  ...
  , ppVisible = xmobarColor base05 "" . clickable wsIconFull
  , ppVisibleNoWindows = Just (xmobarColor basefg "" . clickable wsIconFull)
  , ppHidden = xmobarColor basefg "" . clickable wsIconHidden
  , ppHiddenNoWindows = xmobarColor base05 "" . clickable wsIconEmpty
...
myLayoutHook =
  avoidStruts $ mouseResize $ windowArrange $ windowNavigation $ T.toggleLayouts floats $ mkToggle
...
myManageHook = composeAll
  [ className =? "Brave-browser" --> doShift "0_4"
  , className =? "Mozzilla Firefox" --> doShift "0_4"
  , className =? "Virt-manager" --> doShift "0_4"
  , className =? "obsidian" --> doShift "0_1"
  , className =? "VSCodium" --> doShift "0_2"
  , className =? "zoom" --> doShift "0_7"
  , className =? "discord" --> doShift "0_8"
...
myAdditionalKeys =
  ...
  , ("M-S-w", onNextNeighbour def W.shift)
        -- , ("M-e", onPrevNeighbour def W.view)
...
main = do
  ...
  xmonad
    ...
    $ def
        ...
        , modMask = myModMask
        , terminal = myTerminal
        , startupHook = myStartupHook <+> if nScreens > 1
                          then spawn "xrandr --output HDMI-0 --auto --left-of eDP-1-1"
                          else spawn "xrandr --output HDMI-0 --off"
        , keys = myKeys
