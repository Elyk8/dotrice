...
myLayoutHook =
        avoidStruts $ mouseResize $ windowArrange $ windowNavigation $ T.toggleLayouts floats
...
main  = do
        ...
        xmonad . ewmh . docks . dynamicSBs myStatusBarSpawner $ def {
          manageHook         = myManageHook <+> manageDocks
        , focusFollowsMouse  = True
        , clickJustFocuses   = False
        , modMask            = myModMask
        , terminal           = myTerminal
        , startupHook        = myStartupHook <+> if nScreens > 1 then spawn "xrandr --output HDMI-0 --auto --left-of eDP-1-1" else spawn "xrandr --output HDMI-0 --off"
        , keys               = myKeys
